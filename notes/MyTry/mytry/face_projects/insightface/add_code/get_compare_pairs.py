...
def get_pairs(tree, ide_model, known_vector_dir, k, output, threshold, batch_size, tree_path, outdir):
	print('Identifing!')
	if exists(tree_path):
	...
	pairs = []
	for test_img in tree.test_imgs():
		test_path = test_img.path()
		test_dir_path, test_file_name = split(test_path)
		test_file_idx = file_idx(test_file_name)
		test_dir = split(test_dir_path)[1]
		# ntest_path = join(outdir, test_dir, test_file_name)
		...
		for person in persons:
			paths = ide_model.idx2path[str(person.idx())]
			for path in paths:
				dir_path, file_name = split(path)
				idx = file_idx(file_name)
				dir_name = split(dir_path)[1]
				# npath = join(outdir, dir_name, file_name)
				pairs.append((test_dir, test_file_idx, dir_name, idx))
		# pairs.extend(_pairs)
	
	n_pair = len(pairs)
	print('n_pair: ', n_pair)
	pair_batch_size = 40000
	n_batch = get_batch_number(n_pair, pair_batch_size)
	print('n_batch: ', n_batch)
	for batch_idx in range(n_batch):
		s, e = get_slice_of_batch(n_pair, pair_batch_size, batch_idx)
		print('s,e: ', s, e)
		_batch = pairs[s:e]
		with open(join(outdir, 'pairs_'+ str(batch_idx) +'.txt'), 'w') as f:
			f.write('Hello\n')
			for i, pair in enumerate(_batch):
				f.write(' '.join(pair))
				if i < len(_batch) - 1:
					f.write('\n')
...
if __name__=='__main__':
	...
	get_pairs(tree, ide_model, args['known_vector_dir'], args['k'], args['output'], args['threshold'], args['batch_size'], args['tree_path'], args['outdir'])
