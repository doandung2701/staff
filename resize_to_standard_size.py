...
def create_img(img_resolution, is_value_is_zero = True):
	...
	return img
def resize_img(img, cvt_area, standard_h=None):
    ...
    if standard_h is not None:
        cvt_height = standard_h
...
def resize(img_path, standard_size, back_ground_img=None):
    ...
    if resized_w <= standard_w:
        resized_img = resize_img(img, resized_w*resized_h, standard_h = standard_h)
