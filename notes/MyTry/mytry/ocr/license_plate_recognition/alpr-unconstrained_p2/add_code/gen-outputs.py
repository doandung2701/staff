...
for img_file in img_files:
	...
	lp_paths = glob(os.path.join(output_dir,bname+'*lp.png'))
	# if Lcar:
	...
	for i, lp_path in enumerate(lp_paths):
		...
		lp_label 		= lp_path
		root, filename = os.path.split(lp_path)[:2]
		lp_label_str 	= os.path.join(root,os.path.splitext(filename)[0] + '_str.txt')
